...
createRestaurantHTML = (restaurant) => {
  ...
  if(restaurant.is_favorite == false || restaurant.is_favorite == 'false') 
    ...
    icon.innerHTML = '&#9829;';
  icon.setAttribute('aria-label', `Click to mark ${restaurant.name} as favorite`);
  icon.setAttribute('role','button');
  icon.className = 'favorite';
  icon.addEventListener('click', function() {
    if(icon.innerHTML.charCodeAt(0) == 9825) {
      icon.innerHTML = '&#9829;';
  ...
  neighborhood.innerHTML = `<i class="material-icons">room</i>`+restaurant.neighborhood;
  neighborhood.setAttribute('aria-label',`Neighborhood for the mentioned restaurant is ${restaurant.neighborhood}`);
  neighborhood.tabIndex = 0;
  li.append(neighborhood);

  const address = document.createElement('p');
  address.setAttribute('aria-label',`Address of the mentioned restaurant is ${restaurant.address}`);
  address.innerHTML = restaurant.address;
  address.tabIndex = 0;
  li.append(address);

  const more = document.createElement('a');
  more.innerHTML = 'View Details'+`<i class="material-icons">open_in_new</i>`;
  more.href = DBHelper.urlForRestaurant(restaurant);
  more.setAttribute('role','link');
  more.setAttribute('aria-label',`Enter to know more about ${restaurant.name} restaurant`);
  li.append(more)
  return li
}
